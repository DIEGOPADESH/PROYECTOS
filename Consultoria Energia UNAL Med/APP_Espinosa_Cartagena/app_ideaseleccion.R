            ...
            sidebarLayout(
              sidebarPanel(
                fileInput("voltage_debug_files",
                          "Sube aquí uno o más archivos Excel",
                          multiple = TRUE,
                          accept = c('.csv', ".xlsx")),
                selectInput("voltage_debug_selected",
                            "Selecciona la base que deseas analizar:",
                            choices = NULL)
              ),
              mainPanel(
                tabsetPanel(
...
server <- function(input, output,session) {
  ...
  # Reactivos globales
  bases_cargadas <- reactiveVal(list())
  baseDepurada <- reactiveVal(NULL)
  tipoBase <- reactiveVal("ninguno")
  
  # Subida y selección de bases
  observeEvent(input$voltage_debug_files, {
    req(input$voltage_debug_files)
    archivos <- input$voltage_debug_files
    lista_bases <- list()
    
    for (i in 1:nrow(archivos)) {
      nombre <- archivos$name[i]
      datavolt <- readxl::read_excel(archivos$datapath[i], skip = 1) %>% janitor::clean_names()
      lista_bases[[nombre]] <- datavolt
    }
    
    bases_cargadas(lista_bases)
    updateSelectInput(session, "voltage_debug_selected", choices = names(lista_bases))
  })
  
  # Obtener base seleccionada
  base_actual <- reactive({
    req(bases_cargadas())
    req(input$voltage_debug_selected)
    bases_cargadas()[[input$voltage_debug_selected]]
  })
  
  # Mostrar nombre base seleccionada
  output$voltage_debug_selectfile <- renderUI({
    req(input$voltage_debug_selected)
    h5(strong("Base seleccionada:"), input$voltage_debug_selected)
  })
  
  # Procesamiento de base actual
  observeEvent(base_actual(), {
    base <- base_actual()
    nombres_limpios <- janitor::make_clean_names(names(base))
    names(base) <- nombres_limpios
    ...
    bloque <- str_to_upper(str_extract(input$voltage_debug_selected, "(?<=CALE-)[^-]+"))
    base$Bloque <- bloque
    
    baseDepurada(base)
  })
  
  # Mostrar base original y depurada
  ...
  output$voltage_debug_BL <- renderValueBox({
    ...
    valueBox(
      "Edificación:",
      paste0(unique(baseDepurada()$Bloque), collapse = ", "),
      icon = icon("building"),
      color = "light-blue"
    )
  })
  ...
  output$voltage_debug_inicio <- renderValueBox({
    req(baseDepurada())
    fecha_inicio <- if ("fecha" %in% names(baseDepurada())) {
      min(baseDepurada()$fecha, na.rm = TRUE)
    } else { NA }
    
    valueBox(
      "Fecha de inicio:",
      if (!is.na(fecha_inicio)) format(fecha_inicio, "%Y-%m-%d %H:%M:%S") else "No disponible",
  ...
  output$voltage_debug_fin <- renderValueBox({
    req(baseDepurada())
    fecha_fin <- if ("fecha" %in% names(baseDepurada())) {
      max(baseDepurada()$fecha, na.rm = TRUE)
    } else { NA }
    
    valueBox(
      "Fecha de fin:",
      if (!is.na(fecha_fin)) format(fecha_fin, "%Y-%m-%d %H:%M:%S") else "No disponible",
  ...
  baseFiltrada <- reactive({
    req(baseDepurada())
    req(input$voltage_date_start, input$voltage_date_end)
    
    base <- baseDepurada()
    if (!"fecha" %in% names(base)) return(NULL)
    
    fecha_inicio <- as.POSIXct(input$voltage_date_start)
    fecha_fin <- as.POSIXct(input$voltage_date_end + 1)
    
    base %>% filter(fecha >= fecha_inicio & fecha <= fecha_fin)
  })
  
  output$voltage_debug_filtrado <- renderDT({
    req(baseFiltrada())
    datatable(baseFiltrada(), options = list(pageLength = 10, scrollX = TRUE))
  })
  ...
  datos_procesados <- reactive({
    ...
    if (tipoBase() == "armonico") {
      base_long <- base %>%
        pivot_longer(cols = starts_with("Arm"), names_to = "variable", values_to = "valor") %>%
        mutate(
          armonico = str_extract(variable, "Arm\\d+"),
          fase = str_extract(variable, "Fase\\d")
        )
      list(tipo = "armonico", datos = base_long)
      
    } else if (tipoBase() == "total") {
      base_long <- base %>%
        pivot_longer(cols = starts_with("TotalARM"), names_to = "fase", values_to = "valor") %>%
        mutate(fase = str_extract(fase, "Fase\\d"))
      list(tipo = "thd", datos = base_long)
      
    } else {
      list(tipo = "ninguno", datos = NULL)
    }
  })
  ...
  output$graficos_ui <- renderUI({
    ...
    info <- datos_procesados()
    
    if (info$tipo == "armonico") {
      armonicos <- unique(info$datos$armonico)
      plot_output_list <- lapply(armonicos, function(arm) {
        plotname <- paste0("plot_", arm)
        box(title = paste("Gráfico Armónico", arm),
            plotlyOutput(plotname, height = "400px"),
            width = 12, status = "primary", solidHeader = TRUE, collapsible = TRUE)
      })
      do.call(tagList, plot_output_list)
      
    } else if (info$tipo == "thd") {
      box(title = "Gráfico THD Consolidado",
          plotlyOutput("grafico_thd", height = "400px"),
          width = 12, status = "primary", solidHeader = TRUE, collapsible = TRUE)
    } else {
      h4("No hay datos disponibles para graficar.")
    }
  })
  
  output$grafico_thd <- renderPlotly({
    req(datos_procesados())
    info <- datos_procesados()
    
    if (info$tipo == "thd") {
      p <- ggplot(info$datos, aes(x = fecha, y = valor, color = fase)) +
        geom_line(linewidth = 1) +
        geom_hline(yintercept = limitesSeleccionados()$thd, linetype = "dashed", color = "red") +
        labs(title = "THD Consolidado por Fase", x = "Fecha", y = "THD (%)", color = "Fase") +
        theme_minimal()
      ggplotly(p) %>% layout(legend = list(title = list(text = "Fase")))
    }
  })
  ...
  observe({
    ...
    if (info$tipo == "armonico") {
      ...
      lapply(armonicos, function(arm) {
        local({
          arm_local <- arm
          output[[paste0("plot_", arm_local)]] <- renderPlotly({
            data_arm <- info$datos %>% filter(armonico == arm_local)
            p <- ggplot(data_arm, aes(x = fecha, y = valor, color = fase)) +
              geom_line(linewidth = 1) +
              geom_hline(yintercept = limitesSeleccionados()$armonico, linetype = "dashed", color = "red") +
              labs(title = paste("Gráfico Armónico", arm_local), x = "Fecha", y = "Distorsión (%)", color = "Fase") +
              theme_minimal()
            ggplotly(p) %>% layout(legend = list(title = list(text = "Fase")))
          })
        })
