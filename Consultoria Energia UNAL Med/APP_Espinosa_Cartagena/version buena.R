...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(id = "sidebarId",
                menuItem("Consumo de Energía", tabName = "tab_analisis",icon = icon("bolt"),
                ...
                menuItem("Análisis de la corriente", tabName = "tab_corr", icon = icon("random")),
                menuItem("Calidad de energía", tabName = "tab_cale", icon = icon("plug-circle-check"),
    ...
    # Créditos de creación y modificación de la app:
    tags$footer()
  ),
  ...
  "))
    ...
    # ------Creación de pestaña de Calidad de Energía--------
    ## Creación de pestaña de cargue y depuración
    ...
    # De aca para abajo es lo de la pestaña de orden#
    # Creación de pestaña de análisis de DA Orden 
...
# -----------cortes------------  
 # Dataset reactivo
  ...
  # ----------------------Calidad de Energía------------------
  # Cargue y depuración para base de datos de calidad de energía
  ...
  # Para mostar edificaciones en las que se hará el análisis:
  output$BL <- renderValueBox({
    valueBox(
      "Edificación:",
      HTML(paste("Se analizarán los bloques:<br>", paste(unique(datasetor1()[,11]),collapse = ", "))),
      icon = icon("building"),
      color = "light-blue"
    )
  })
  ...
  # Para mostar fecha de inicio de registros de la base de datos
  output$inicio_reg1 <- renderValueBox({
    valueBox(
      "Inicio:",
      HTML(paste0("La base de datos contiene registros desde:<br>", aviso(debug_BD(cale())[[2]]))),
      icon = icon("calendar-minus"),
      color = "light-blue" 
    )
  })
  ...
  # Para mostar fecha de fin de registros de la base de datos
  output$fin_reg1 <- renderValueBox({
    valueBox(
      "Fin:",
      HTML(paste0("La base de datos contiene registros hasta:<br>",aviso(debug_BD(cale())[[3]]))),
      icon = icon("calendar-plus"),
      color = "light-blue" 
    )
  })
  
  # Datos por bloque (aca se selecciona las variables)
  arm_BL<- reactive({
  ...
  #De aca para abajo se cambia codigo de los grafico de calidad de energia # 
